...
create or replace temp view  2_day_accounts_pop
as
...
cache table 2_day_accounts_pop;

-- COMMAND ----------

select count(*) from 2_day_accounts_pop
...
from dwhdb.customers c
   ...
   join    2_day_accounts_pop ac on ac.Accountnumber=c.AccountNumber ;
...
from dwhdb.events e
join population_data p on  (p.AccountNumber = e.AccountNumber) and  e.Event_Date_day<= date_add(p.FirstDepositday,1) 
where Source='fxnet'
and Event_Name='deal'
and Event_Date_day>='2019-01-01'
and Event_Date_day>=date_add(now(), -35)  ; 
...
select count(*) from 1_days_trading_activity
...
-- MAGIC join 2_day_accounts_pop ac on ac.Accountnumber = qu.AccountNumber
...
select  
       ...
       c.trading_days,
       d.FirstDepositDate,
       d.deposit_amount_1_days_after_deposit,
       d.deposit_events_1_Days_after_Deposit,
...
-- DBTITLE 1,Add new Customers Data to Clustering Model 2 Days Customers_Data table
MERGE INTO dwhdb.Clustering_Model_2_Days_Customers_Data e
...
OPTIMIZE dwhdb.Clustering_Model_2_Days_Customers_Data;
set spark.databricks.delta.retentionDurationCheck.enabled = false;
VACUUM   dwhdb.Clustering_Model_2_Days_Customers_Data RETAIN 0 HOURS;

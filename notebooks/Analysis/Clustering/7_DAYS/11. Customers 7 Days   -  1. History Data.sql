...
WHERE FolderType like 'Live'
    and   CAST(get_json_object(details, "$.FirstDepositDay")AS DATE) >='2018-01-01' 
    and   CAST(get_json_object(details, "$.FirstDepositDay")AS DATE) <='2018-10-20'
    and   CAST(get_json_object(details, "$.FirstDealDate")AS DATE) >='2018-01-01';
...
and Event_Details='Deposit'
 ...
 select AccountNumber
    ...
    --- 6 days 
     ,min(case when Event_Date>FirstDepositDate and Event_Date<date_add(FirstDepositday,7) then Event_Date end) as first_deposit_Date_6_Days_after_first
     ,sum(case when Event_Date>FirstDepositDate and  Event_Date<date_add(FirstDepositday,7)then Event_Sum else 0 end) as deposit_amount_6_days_after_deposit
     ,sum(case when Event_Date>FirstDepositDate and Event_Date<date_add(FirstDepositday,7) then 1 else 0 end )       as deposit_events_6_Days_after_Deposit  
    --- 30 days
       ,min(case when Event_Date>FirstDepositDate and Event_Date<date_add(FirstDepositday,31) then Event_Date end) as first_deposit_Date_30_Days_after_first
     ,sum(case when Event_Date>FirstDepositDate and  Event_Date<date_add(FirstDepositday,31)then Event_Sum else 0 end) as deposit_amount_30_days_after_deposit
     ,sum(case when Event_Date>FirstDepositDate and Event_Date<date_add(FirstDepositday,31) then 1 else 0 end )       as deposit_events_30_Days_after_Deposit  
...
where  AccountNumber=5419195

-- COMMAND ----------

-- DBTITLE 1,get data for calculate 7 days activity after deposit 
create or replace temp view  7_days_trading_activity
as
select 
     e.AccountNumber
      ,e.Event_Details
   ...
   -- ,e.* 
from dwhdb.events e
join population p on  (p.AccountNumber = e.AccountNumber) and  e.Event_Date_day<= date_add(p.FirstDepositday,6) 
where Source='fxnet'
and Event_Name='deal'
and Event_Date_day>'2018-01-01'  ; 
  
cache table 7_days_trading_activity
...
from 6_days_trading_activity a
...
(
select d.*,
        case when (DATEDIFF(last_deal_date,first_deal_date)=0)  then 7
             when trading_days=1 then 7
             ELSE DATEDIFF(last_deal_date,first_deal_date)/(trading_days-1) end as avg_days_between_trading_days
      , deposit_amount_6_days_after_deposit+first_deposit_amount     as IPA_7_DAYS
...
-- COMMAND ----------
 ...
 drop table dwhdb.Models_Customers_Data_7_Days

-- COMMAND ----------

create table dwhdb.Models_Customers_Data_7_Days
USING DELTA
select  * from Customers_data_for_all_insert

-- COMMAND ----------

OPTIMIZE dwhdb.Models_Customers_Data_7_Days;
set spark.databricks.delta.retentionDurationCheck.enabled = false;
VACUUM   dwhdb.Models_Customers_Data_7_Days RETAIN 0 HOURS;

-- COMMAND ----------


--MERGE INTO dwhdb.Models_Customers_Data_7_Days e
...
select count(*) from dwhdb.Models_Customers_Data_7_Days

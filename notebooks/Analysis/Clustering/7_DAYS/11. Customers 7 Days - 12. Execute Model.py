...
# MAGIC %sql
# MAGIC refresh table dwhdb.Models_Execution
...
# MAGIC  --and First_Deposit_Date>='2019-01-01'
...
# MAGIC %sql
# MAGIC refresh table accounts_pop
...
# MAGIC       ,deposit_amount_6_days_after_deposit	
# MAGIC       ,deposit_events_6_Days_after_Deposit
# MAGIC       ,IPA_7_DAYS	
...
# MAGIC from dwhdb.Clustering_Model_7_Days_Customers_Data da
# MAGIC join accounts_pop ac on ac.Accountnumber=da.Accountnumber

# COMMAND ----------

# MAGIC %sql
# MAGIC select * from new_customers_data

# COMMAND ----------

# DBTITLE 1,Get  data for quantile to pandas data frame
df_quantile=sqlContext.sql('SELECT * FROM dwhdb.7_day_cluster_models_quantile').toPandas()
df_folder=sqlContext.sql('SELECT * FROM dwhdb.7_day_cluster_models_folder_mean').toPandas()
df_Q_Occupation=sqlContext.sql('SELECT * FROM dwhdb.7_day_cluster_models_Q_Occupation_mean').toPandas()
...
# DBTITLE 1,quantile
pddf['IPA_7_DAYS_quantile']=np.where(pddf.IPA_7_DAYS<ipa_95_quantile,pddf.IPA_7_DAYS,ipa_95_quantile)
...
pddf['folder_mean'] = pddf.folder_mean.replace(np.NaN, 398.4792623) 
...
clusters_7_Days=df_no_bronze[['acc','IPA_7_DAYS_quantile','trading_days','Q_Occupation_mean','folder_mean',
                    'number_of_deals_quantile','unique_instruments_traded_quantile','number_of_deposit_quantile'
                    ]]
...
# DBTITLE 1,Customers 7 Days model
...
display (clusters_7_Days.groupby('Cluster_kmeans').mean())
...
cluster_analysis = pd.merge(df_Activity, clusters_7_Days[['Cluster_kmeans']], how='left', on=['acc'])
...
pddf['cluster_ipa']=np.where((pddf.IPA_7_DAYS<=101) & (pddf.folder!="JP-LIVE") & (pddf.folder!="INDIA-LIVE") 
...
# MAGIC as
# MAGIC select Accountnumber,207 as Model_ID , now() as Model_Execution_Date,  to_json(named_struct('Cluster_7_day', Cluster_final)) as Model_Result
...
# MAGIC VACUUM   dwhdb.Models_Execution RETAIN 25 HOURS;

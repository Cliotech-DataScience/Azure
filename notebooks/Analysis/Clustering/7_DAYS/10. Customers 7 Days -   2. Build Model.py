...
# MAGIC        d.deposit_amount_6_days_after_deposit,
# MAGIC        d.deposit_events_6_Days_after_Deposit,
# MAGIC        d.IPA_7_DAYS,
# MAGIC        ---questions_
...
# MAGIC from dwhdb.Models_Customers_Data_7_Days  d
...
pddf['deposit_events']=pddf.deposit_events_6_Days_after_Deposit+1

# COMMAND ----------

# DBTITLE 1,quantile 
IPA_7_DAYS_qunt_95=pddf.IPA_7_DAYS.quantile(0.95)
...
pddf['IPA_7_DAYS_quantile']=np.where(pddf.IPA_7_DAYS<pddf.IPA_7_DAYS.quantile(0.95),pddf.IPA_7_DAYS,pddf.IPA_7_DAYS.quantile(0.95))
...
print ("IPA 7 DAYS quantile(0.95)                is ..",IPA_7_DAYS_qunt_95) 
...
IPA_7_DAYS_quantile_95=pddf.IPA_7_DAYS.quantile(0.95)
...
quantiles = [['ipa', IPA_7_DAYS_quantile_95], ['deposit', number_of_deposit_quantile_95], ['instrument', unique_instruments_traded_quantile_95],['deals',number_of_deals_quantile_95] ]
...
# MAGIC  %sql
# MAGIC   CREATE TABLE dwhdb.7_day_cluster_models_quantile
# MAGIC   USING delta
...
# MAGIC %sql
# MAGIC   OPTIMIZE dwhdb.7_day_cluster_models_quantile;
...
# MAGIC  %sql
# MAGIC   CREATE TABLE dwhdb.7_day_cluster_models_Q_Occupation_mean
# MAGIC   USING delta
# MAGIC   AS SELECT * FROM Q_Occupation_mean ; 
# MAGIC   
# MAGIC OPTIMIZE  dwhdb.7_day_cluster_models_Q_Occupation_mean ;
# MAGIC set spark.databricks.delta.retentionDurationCheck.enabled = false;
# MAGIC VACUUM   dwhdb.7_day_cluster_models_Q_Occupation_mean RETAIN 0 HOURS;
# MAGIC    

# COMMAND ----------

# MAGIC  %sql
# MAGIC CREATE TABLE dwhdb.7_day_cluster_models_folder_mean
# MAGIC   USING delta
...
# MAGIC  %sql
# MAGIC   OPTIMIZE dwhdb.7_day_cluster_models_folder_mean ;
# MAGIC set spark.databricks.delta.retentionDurationCheck.enabled = false;
# MAGIC VACUUM  dwhdb.7_day_cluster_models_folder_mean RETAIN 0 HOURS;

# COMMAND ----------

# MAGIC %sql
# MAGIC select * from dwhdb.7_day_cluster_models_folder_mean
...
bronze=df_Activity[(df_Activity.trading_days<=2) & (df_Activity.deposit_events<=1) & (df_Activity.unique_instruments_traded_quantile<=2)
    & (df_Activity.IPA_7_DAYS<=100)]
...
df_no_bronze=df_Activity[(df_Activity.trading_days>2) | (df_Activity.deposit_events>1) | (df_Activity.unique_instruments_traded_quantile>2)
    | (df_Activity.IPA_7_DAYS>101)]
...
clusters_7_Days=df_no_bronze[['acc','number_of_deals','IPA_7_DAYS_quantile','folder_mean','trading_days','unique_instruments_traded_quantile',
                             'number_of_deposit_quantile']]

# COMMAND ----------

clusters_7_Days.isnull().sum()

# COMMAND ----------

clusters_7_Days=clusters_7_Days.dropna()
clusters_7_Days=clusters_7_Days.set_index('acc') 
...
# normalize data 
from sklearn import preprocessing
normalized_data = preprocessing.normalize(clusters_7_Days)

# COMMAND ----------

# k means clustering 

kmeans_7_days = KMeans(n_clusters=2 ,random_state=123)
predictions = kmeans_7_days.fit_predict(normalized_data) 

# COMMAND ----------

clusters_7_Days['Cluster_kmeans'] = predictions

# COMMAND ----------

clusters_7_Days['Cluster_kmeans'].value_counts()

# COMMAND ----------

# DBTITLE 1,fixed order of clusters
res_0=clusters_7_Days[clusters_7_Days.Cluster_kmeans==0]
res_1=clusters_7_Days[clusters_7_Days.Cluster_kmeans==1]
avg_0=res_0.IPA_7_DAYS_quantile.mean()
avg_1=res_1.IPA_7_DAYS_quantile.mean()
if (avg_0>avg_1):
  clusters_7_Days['Cluster_kmeans'] = clusters_7_Days['Cluster_kmeans'].replace(0,7)
else:
  clusters_7_Days['Cluster_kmeans'] = clusters_7_Days['Cluster_kmeans'].replace(1,7)
...
cluster_analysis = pd.merge(df_Activity,
                   clusters_7_Days[['Cluster_kmeans']]
                  ,how='left'
                  ,on='acc')
...
pddf['cluster_ipa']=np.where((pddf.IPA_7_DAYS<=100) & (pddf.folder!="JP-LIVE") & (pddf.folder!="INDIA-LIVE") 
                           ...
                           ,"Bronze",
                          np.where(pddf.IPA_7_DAYS<=101,"Silver",
                          np.where(pddf.IPA_7_DAYS<=500,
                                   "Silver","Gold"
                          )))
...
# MAGIC as
# MAGIC    select AccountNumber,210 as ModelID, 'Customers_Clustering_7_Days'as Model_Name,now()  as update_date 
# MAGIC        ,Cluster_final 
...
# MAGIC  %sql
# MAGIC   CREATE TABLE avi.Customers_cluster_model_7
# MAGIC   USING delta
# MAGIC  
...
# MAGIC %sql
# MAGIC OPTIMIZE avi.Customers_cluster_model;
# MAGIC set spark.databricks.delta.retentionDurationCheck.enabled = false;
# MAGIC VACUUM  avi.Customers_cluster_model RETAIN 0 HOURS;

# COMMAND ----------

# DBTITLE 1,Save Clustering Model
filename = '/dbfs/mnt/dwhdbstore/models/Clustering//Customers_7_Days_Model.pickle'
pickle.dump(kmeans_7_days, open(filename, 'wb'))

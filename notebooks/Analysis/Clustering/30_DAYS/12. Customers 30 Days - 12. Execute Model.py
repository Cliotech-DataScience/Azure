...
# MAGIC       ,deposit_amount_30_days_after_deposit	
# MAGIC       ,deposit_events_30_Days_after_Deposit
# MAGIC       ,days_from_last_deposit
# MAGIC       ,days_from_last_trading_Day
# MAGIC       ,IPA_30_DAYS	
...
# MAGIC from dwhdb.Clustering_Model_30_Days_Customers_Data da
# MAGIC join accounts_pop ac on ac.Accountnumber=da.Accountnumber

# COMMAND ----------

# MAGIC %sql
# MAGIC select count(*) from new_customers_data

# COMMAND ----------

# DBTITLE 1,Get  data for quantile to pandas data frame
df_quantile=sqlContext.sql('SELECT * FROM dwhdb.30_day_cluster_models_quantile').toPandas()
df_folder=sqlContext.sql('SELECT * FROM dwhdb.30_day_cluster_models_folder_mean').toPandas()
df_Q_Occupation=sqlContext.sql('SELECT * FROM dwhdb.30_day_cluster_models_Q_Occupation_mean').toPandas()
...
# DBTITLE 1,quantile
pddf['IPA_30_DAYS_quantile']=np.where(pddf.IPA_30_DAYS<ipa_95_quantile,pddf.IPA_30_DAYS,ipa_95_quantile)
...
pddf['folder_mean'] = pddf.folder_mean.replace(np.NaN, 647.8782623)
...
len(clusters_30_Days)

# COMMAND ----------

display(clusters_30_Days)
...
# DBTITLE 1,Customers 30 Days model
...
display (clusters_30_Days.groupby('Cluster_kmeans').mean())
...
cluster_analysis = pd.merge(df_Activity, clusters_30_Days[['Cluster_kmeans']], how='left', on=['acc'])
...
pddf['cluster_ipa']=np.where((pddf.IPA_30_DAYS<=101) & (pddf.folder!="JP-LIVE") & (pddf.folder!="INDIA-LIVE") 
                           ...
                           ,"Bronze",
                          np.where(pddf.IPA_30_DAYS<=100,"Silver",
...
# MAGIC as
# MAGIC select Accountnumber,230 as Model_ID , now() as Model_Execution_Date,  to_json(named_struct('Cluster_30_day', Cluster_final)) as Model_Result
